![cc-fleet — 把任意三方模型变成真实 Claude agent,在 Claude Code 或 Codex 里跑 Dynamic Workflow、Agent Team、Subagent](assets/cc-fleet-banner.png)

<h1 align="center">🚢 cc-fleet</h1>

<p align="center"><strong>🤖 cc-fleet 把任意三方模型 — DeepSeek · GLM · Kimi · Qwen…… — 变成真实 Claude agent,在 Claude Code 或 Codex 里用你的订阅驱动 ⚙️ Dynamic Workflow、👥 Agent Team、⚡ Subagent 🚀</strong></p>
...
每个第三方 worker 都是一个真实的 `claude` 进程,只是 LLM 后端换成了对应服务商,Claude Code 用驱动原生 Agent 的方式驱动它即可。主会话自己的认证(OAuth 订阅或 API key)不受影响,第三方 key 不会进入环境变量、命令行参数或 shell 历史 — **无泄漏风险**。

**上手两步**:一键安装,配置一个 Provider。之后在 Claude Code 里用 `/workflow`、`/team`、`/subagent` 指定意图,或直接用自然语言描述任务 — 意图识别与 CLI 调用全部由 Claude 自动完成。

如果没有 Claude 订阅,`ccf run <provider>` 直接启动一个由该 Provider 驱动的交互式会话 — **还是熟悉的 `claude`**,只是跑在对应的 Provider 模型上。

cc-fleet 也能被 Codex 驱动:一个 Codex 插件让 Codex 会话获得同样的扇出 — `cc-fleet subagent` 与 `cc-fleet workflow`,由 `claude` 引擎当 worker。(Teammate 仍只在 Claude Code 内。)
...
**Codex 插件** — 要从 Codex 会话驱动 cc-fleet,装完上面的 cc-fleet 后运行:
...
> 其他安装方式(npm / go install / Releases / 源码)与 Claude Code 插件补装、安装器覆盖参数、环境要求与维护,见 **[安装与维护](install.zh.md)**。
...
<table>
<tr>
<td width="50%" align="center"><img src="assets/workflow-board.webp" width="100%" /><br/><sub>Agents Board:phase → leaf 进度树</sub></td>
<td width="50%" align="center"><img src="assets/workflow-leaf.webp" width="100%" /><br/><sub>钻进单个 leaf:完整 prompt 与汇总输出</sub></td>
</tr>
</table>
...
<table>
<tr>
<td width="50%" align="center"><img src="assets/team-panes.webp" width="100%" /><br/><sub>tmux 里四个 Teammate 并行工作</sub></td>
...
<table>
<tr>
<td width="50%" align="center"><img src="assets/subagent-fanout.webp" width="100%" /><br/><sub>一句话并行分出三个 Subagent</sub></td>
<td width="50%" align="center"><img src="assets/subagent-board.webp" width="100%" /><br/><sub>看板里的 job 列表与详情</sub></td>
</tr>
</table>
...
cc-fleet 也能插进 **OpenAI Codex**:一个 Codex 插件让 Codex 会话扇出同样的 Provider worker — 一次性或批量 **subagent**(`cc-fleet subagent`)与多阶段 JS **workflow**(`cc-fleet workflow`),每个都是跑在 Provider 模型上的真实 `claude` 进程。Teammate lane 仍只在 Claude Code 内;从 Codex 你得到 subagent 与 workflow 扇出。(安装见上面的 [快速安装](#快速安装)。)
...
<table>
<tr>
<td width="50%" align="center"><img src="assets/provider-presets.webp" width="100%" /><br/><sub>可选多家厂商预设</sub></td>
<td width="50%" align="center"><img src="assets/provider-config.webp" width="100%" /><br/><sub>配置各档位模型思考强度与权限</sub></td>
</tr>
</table>
